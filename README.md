...
## Reading layer `PROVCenso2010' from data source `/home/acade/unidad-2-asignacion-1-esda-operaciones-con-atributos-plots-acadetev/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
proven17 %>%
  dplyr::select(contains('Grupo Socio-Económico')) %>%
  plot(breaks = 'jenks')
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('Grupo Socio-Económico'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
...
``` r
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('Grupo Socio-Económico'))
##                 TOPONIMIA Grupo Socio-Económico: Muy bajo
## 1       DISTRITO NACIONAL                             102
## 2                    AZUA                              82
## 3                 BAORUCO                              67
## 4                BARAHONA                             105
## 5                 DAJABÓN                              25
...
## 12              LA ROMANA                              50
## 13                LA VEGA                              72
## 14 MARÍA TRINIDAD SÁNCHEZ                              24
## 15           MONTE CRISTI                              75
## 16             PEDERNALES                              20
## 17                PERAVIA                              36
## 18           PUERTO PLATA                              50
## 19       HERMANAS MIRABAL                              12
## 20                 SAMANÁ                              27
## 21          SAN CRISTÓBAL                              75
## 22               SAN JUAN                              97
## 23   SAN PEDRO DE MACORÍS                              86
## 24        SANCHEZ RAMÍREZ                              33
## 25               SANTIAGO                             114
## 26     SANTIAGO RODRÍGUEZ                              29
## 27               VALVERDE                             117
## 28         MONSEÑOR NOUEL                              13
## 29            MONTE PLATA                              61
## 30             HATO MAYOR                              27
## 31       SAN JOSÉ DE OCOA                              26
## 32          SANTO DOMINGO                             170
##    Grupo Socio-Económico: Bajo Grupo Socio-Económico: Medio bajo
## 1                          360                              1078
## 2                          206                               350
## 3                           82                               139
...
Grupo Socio-Económico: Bajo
</th>
<th style="text-align:right;">
Grupo Socio-Económico: Medio
</th>
<th style="text-align:right;">
Grupo Socio-Económico: Medio alto-Alto
</th>
<th style="text-align:right;">
Grupo Socio-Económico: Medio bajo
</th>
<th style="text-align:right;">
Grupo Socio-Económico: Muy bajo
</th>
</tr>
</thead>
<tbody>
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'Grupo Socio-Económico: Medio alto-Alto') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
proven17 %>% st_drop_geometry() %>%
  dplyr::select(contains('Grupo Socio-Económico')) %>% 
  summary
##  Grupo Socio-Económico: Muy bajo Grupo Socio-Económico: Bajo
##  Min.   : 12.00                  Min.   :  32.0             
##  1st Qu.: 28.50                  1st Qu.:  62.5             
##  Median : 51.50                  Median : 124.0             
##  Mean   : 59.56                  Mean   : 169.2             
##  3rd Qu.: 79.00                  3rd Qu.: 203.0             
##  Max.   :170.00                  Max.   :1079.0             
##  Grupo Socio-Económico: Medio bajo Grupo Socio-Económico: Medio
##  Min.   :  40.0                    Min.   :  13.0              
##  1st Qu.: 136.8                    1st Qu.:  67.5              
##  Median : 257.5                    Median : 132.5              
##  Mean   : 402.5                    Mean   : 388.3              
##  3rd Qu.: 438.2                    3rd Qu.: 378.2              
##  Max.   :2923.0                    Max.   :3655.0              
##  Grupo Socio-Económico: Medio alto-Alto
##  Min.   :   2.0                        
##  1st Qu.:  17.0                        
##  Median :  40.0                        
...
proven17 %>% st_drop_geometry() %>%
  dplyr::select(contains('Grupo Socio-Económico')) %>%
  select_if(is.numeric) %>%  gather(variable, valor) %>%
  ggplot + aes(x=valor) %>% geom_histogram(bins=15) +
...
``` r
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0405') %>% 
  dplyr::select(contains('Grupo Socio-Económico'))
##   Grupo Socio-Económico: Muy bajo Grupo Socio-Económico: Bajo
## 1                              25                          44
##   Grupo Socio-Económico: Medio bajo Grupo Socio-Económico: Medio
